...
class Army {
  ...
  buildLevel(level) {
    ...
    if (level > 1) {
      this.#widthSpeed =
        (config.army.speed.width *
          config.army.speed.level_widthFactor *
          level) /
        2;
      this.#depthSpeed =
        (config.army.speed.depth *
          config.army.speed.level_depthFactor *
          level) /
        2;
      this.#minNextShot =
        config.army.shot.nextShotInterval.min /
        ((level / 2) * config.army.shot.level_shotFreqFactor);
      this.#maxNextShot =
        config.army.shot.nextShotInterval.max /
        ((level / 2) * config.army.shot.level_shotFreqFactor);
      this.#shotSpeedFactor =
        (config.army.shot.level_shotFreqFactor * level) / 2;
    } else if (level == 1) {
  ...
  placeBattalion(nbBattalions, enemiesDepthSize) {
    const battalionSpacingDepth =
      (this.#spaceAvailable - enemiesDepthSize) / (nbBattalions - 1);
    ...
    for (let i = 0; i < nbBattalions; i++) {
      ...
      battalionPosition.set(
        0,
        0,
        battalionPosZ + config.world.size.depth - this.#spaceAvailable
      );
      battalion.setPosition(battalionPosition);

      previousEnemyPositionZ =
        battalionPosZ + battalionDepthSize / 2 + battalionSpacingDepth;
    }
  }
  ...
  tick(delta) {
    ...
    this.#armyGroup.position.x +=
      this.#widthSpeed * this.#widthDirection * delta;
  ...
  upgrade() {
    ...
    newWidthSpeed > config.army.speed.maxWidth
      ? (this.#widthSpeed = config.army.speed.maxWidth)
      : (this.#widthSpeed = newWidthSpeed);

    const newDepthSpeed = this.#depthSpeed * config.army.speed.wave_widthFactor;
    newDepthSpeed > config.army.speed.maxWidth
      ? (this.#depthSpeed = config.army.speed.maxDepth)
      : (this.#depthSpeed = newDepthSpeed);

    const newMinNextShot =
      this.#minNextShot / config.army.shot.wave_shotFreqFactor;
    newMinNextShot < config.army.shot.minShot
      ? (this.#minNextShot = config.army.shot.minShot)
      : (this.#minNextShot = newMinNextShot);
    const newMaxNextShot =
      this.#maxNextShot / config.army.shot.wave_shotFreqFactor;
    newMaxNextShot < config.army.shot.minShot
      ? (this.#maxNextShot = config.army.shot.minShot)
      : (this.#maxNextShot = newMaxNextShot);

    const newShotSpeedFactor =
      this.#shotSpeedFactor * config.army.shot.wave_speedShotFactor;
    newShotSpeedFactor > config.army.shot.maxShotSpeedFactor
      ? (this.#shotSpeedFactor = config.army.shot.maxShotSpeedFactor)
  ...
  collisionWithEnemy(hitContainer, damage) {
    armyBox.setFromObject(this.#armyGroup);
    if (
      (hitContainer instanceof THREE.Box3 &&
        armyBox.intersectsBox(hitContainer)) ||
      (hitContainer instanceof THREE.Sphere &&
        armyBox.intersectsSphere(hitContainer))
    ) {
  ...
  setNextShot() {
    this.#nextShot =
      Math.random() * (this.#maxNextShot - this.#minNextShot) +
      this.#minNextShot;
  }
  ...
  shoot() {
    ...
    if (!this.enemiesDied()) {
      if (currentTime - this.#lastShotTime >= this.#nextShot) {
        ...
        const indexBattalion = Math.floor(
          Math.random() * battalionsAlive.length
        );
        const battalion = battalionsAlive[indexBattalion];

        // On choisit un ennemi vivant au hasard
        const enemiesInBattalion = battalion
          .getEnemies()
          .filter(function (enemy) {
            return !enemy.isDying();
          });
        const indexEnemy = Math.floor(
          Math.random() * enemiesInBattalion.length
        );
  ...
  createBullet(battalion, enemy) {
    ...
    bulletPosition.z =
      this.getPosition().z + battalion.getPosition().z - enemy.getSize().z / 2;
    bulletPosition.x = this.getPosition().x + enemy.getPosition().x;
    const bullet = new Bullet(
      this.#scene,
      "enemy",
      bulletPosition,
      config.enemies[enemy.getType()].bulletSpeed * this.#shotSpeedFactor,
      1
    );
