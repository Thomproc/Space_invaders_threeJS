...
import shipGltf from "../../assets/models/star_sparrow_modular_spaceship.glb";

import kamikazeGlb from "../../assets/models/kamikaze_guardian.glb";
import observerGlb from "../../assets/models/observer_guardian.glb";
import rollerGlb from "../../assets/models/roller_guardian.glb";

import shieldHit0Glb from "../../assets/models/shieldHit0.glb";
import shieldHit1Glb from "../../assets/models/shieldHit1.glb";
import shieldHit2Glb from "../../assets/models/shieldHit2.glb";

import jupiterGlb from "../../assets/models/jupiter.glb";

const guardiansGlb = {
  kamikaze: kamikazeGlb,
  observer: observerGlb,
  roller: rollerGlb,
};
...
class Models {
  ...
  async loadShip() {
    ...
    const shipDatas = await this.#loader.loadAsync(shipGltf);
    ...
    const durations = {
      // Durée de l'animation en ms pour évoluer/uprgade (resp. rétrograder/downgrade) au niveau "key"
      upgrade: {
  ...
  async loadPlanets() {
    const planetDatas = await this.#loader.loadAsync(jupiterGlb);
  ...
  async loadShields() {
    for (const i of Object.keys(this.#shields)) {
      const shieldDatas = await this.#loader.loadAsync(shieldHitGlbs[i]);
  ...
  async loadEnemies() {
    for (const enemyType of Object.keys(this.#enemies)) {
      const modelDatas = await this.#loader.loadAsync(guardiansGlb[enemyType]);
      ...
      this.#enemies[enemyType].size = this.getSize(
        this.#enemies[enemyType].model
      );
    }
  }
  ...
  scaleModel(model, sizeMax) {
    ...
    model.scale.set(
      (1 / modelSize.x) * sizeMax,
      (1 / modelSize.y) * ratioY * sizeMax,
      (1 / modelSize.z) * ratioZ * sizeMax
    );
    return;
  }
  ...
  getEnemy(type) {
    ...
    const clonedClip =
      enemy.datasGLTF.animations[enemy.datasGLTF.animations.length - 1].clone();
