...
class Environment {
  ...
  constructor(scene, loop, models) {
    ...
    for (let i = 0; i < this.#nbStars; i++) {
      const x =
        Math.random() * 4 * config.world.size.width -
        2 * config.world.size.width;
      const y = Math.random() * 30 - 15;
      const z =
        Math.random() * config.world.size.depth - config.world.size.depth / 2;
    ...
    this.#planet.model.position.set(
      0,
      0,
      config.world.size.depth + this.#planet.size.z
    );
    ...
    if (this.#gameIsWin) {
      this.#enemiesDatas.forEach((enemyDatas, index) => {
        ...
        enemyModel.position.y =
          (Math.sin(time) * this.#enemiesFloatingInterval) / 2;
  ...
  switchToVictory() {
    for (let i = 0; i < this.#nbEnemies; i++) {
      ...
      enemyDatas.model.position.set(
        ...
        Math.random() * 2 * this.#enemiesFloatingInterval -
          this.#enemiesFloatingInterval,
        Math.random() *
          (config.world.size.depth - config.world.size.depth / 5) +
          config.world.size.depth / 5
      );
