...
class World {
  ...
  constructor(container) {
    ...
    this.#events.addEvent("keypress", (e) => {
      if (this.#gameIsOn && (e.key === "0" || e.key === "1")) {
        this.#camerasManager.switchCamera(parseInt(e.key));
        this.#resizer.resize();
      } else if (e.key === "m") {
        this.#soundManager.enableOrDisableAmbienceMusic()
          ? this.#IHM.showPopup("Musique d'ambiance activée")
          : this.#IHM.showPopup("Musique d'ambiance désactivée");
      } else if (e.key === "h") {
        this.#IHM.help();
      } else if (
        e.key === "Enter" &&
        (this.#IHM.gameIsOver() || this.#IHM.gameIsWin())
      ) {
